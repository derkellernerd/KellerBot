...
import { type ApiKellerBotAction, KellerBotAction, KellerBotActionType } from 'src/models/keller_bot_action';
...
function save() {
  ...
  Keller_bot.updateAction(action.value.ID, action.value)
    .then(async (result) => {
...
function create() {
  ...
  Keller_bot.createAction(action.value)
    .then(async (result) => {
      if (result.status === 201) {
        action.value = KellerBotAction.fromApi(result.data.Data);
        await uploadFile();
  ...
  <q-page padding>
    <div v-if="action">
      <q-form @submit="createOrSave()">
        <q-card>
          ...
          <q-card-section>
            ...
            <q-select
              ...
              :options="actionTypes"
            />
            <q-file label="File" v-if="isFileAction" v-model="file" @change="fileHasChanged = true"/>
          </q-card-section>
        </q-card>
        ...
        <ActionChatMessageEdit v-if="action.ActionType == KellerBotActionType.ChatMessage" v-model="action"/>
        <ActionCompositionEdit
          v-if="action.ActionType == KellerBotActionType.Composition"
          v-model="action"
        />
        <q-btn type="submit" :icon="isNew ? 'add' : 'save'" :label="isNew ? 'create' : 'save'" class="full-width q-mt-lg" color="positive" />
      </q-form>
    </div>
  </q-page>
</template>

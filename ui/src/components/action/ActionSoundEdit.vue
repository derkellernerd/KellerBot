...
import type { ApiKellerBotActionTypeGif } from 'src/models/keller_bot_action';
...
const actionSound = computed(() => {
  return action.value.actionSound;
});

const durationSeconds = computed({
  get() {
    return actionSound.value.DurationMs / 1000;
  },
  set(val: number) {
    actionSound.value.DurationMs = val * 1000;
  },
});
...
<template>
  <div class="row q-mt-lg" v-if="actionSound">
    ...
    <div class="col-4">
      <q-card>
        <q-card-section class="text-white text-subtitle2 bg-primary"> Preview </q-card-section>
        <q-card-section class="q-pa-none">
          <audio controls
            v-if="actionSound.FileName"
